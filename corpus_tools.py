...
# -*- coding: utf-8 -*-
"""
@author: kkyle2 & Henry Hosseini
version: 0.32 (The version which was expanded by Henry Hosseini)
Source: https://github.com/kristopherkyle/corpus_toolkit/blob/master/corpus_toolkit/corpus_tools.py
"""
...
try:	
	nlp = spacy.load("en_core_web_lg") #load the English model. This can be changed - just make sure that you download the appropriate model first
	...
	for x in collocate_freq:
		...
		if observed < cutoff: #if the collocate frequency doesn't meet the cutoff, ignore it
			...
			if stat == "MI": #pointwise mutual information
				...
				logdice_score = 14.0 + math.log2((2.0 * observed)/(corp_freq[target] + corp_freq[x]))
				stat_dict[x] = logdice_score
...
def nounchunk_dep(corpus, dep, lemma = True, lower = True, pron = False):
	...
	for text in corpus: #iterate through corpus filenames
		...
		for chunk in doc.noun_chunks:
			if chunk.root.dep_ == dep: #if the token's dependency tag matches the one designated
				if lemma == True: #if lemma is true, use lemma form of dependent and head
					if pron == False: #if we don't want Spacy's pronoun lemmatization
						if chunk.lemma_ == "-PRON-":
							dependent = chunk.text.lower() #then use the raw form of the word
							headt = chunk.root.head.text.lower()
						else:
							dependent = chunk.lemma_
							headt = chunk.root.head.lemma_
					else:
						dependent = chunk.lemma_
						headt = chunk.root.head.lemma_
				...
				if lemma == False: #if lemma is false, use the token form
					if lower == True: #if lower is true, lower it
						dependent = chunk.text.lower()
						headt = chunk.root.head.text.lower()
					else: #if lower is false, don't lower
						dependent = chunk.text
						headt = chunk.root.head.text
	...
	bigram_dict = {"bi_freq":bi_freq,"dep_freq":dep_freq,"head_freq": head_freq, "range":range_freq} #create a dictioary of dictionaries
	return(bigram_dict) # return dictionary of dictionaries
...
def soa(freq_dict,stat = "MI", range_cutoff = 5, cutoff=5):
	dummy_zero = 0.00000000000000000001
	stat_dict = {}
	...
	for x in freq_dict["bi_freq"]:
		...
		# except KeyError:
		# 	continue
		dep_freq = freq_dict["dep_freq"].get(dep, dummy_zero) #get dependent frequency from dictionary
		...
		# except KeyError:
		# 	continue
		head_freq = freq_dict["head_freq"].get(head, dummy_zero) #get head frequency from dictionary
		...
		if stat == "MI": #pointwise mutual information
		...
		elif stat == "dp_dep": # (e.g., getting "apple" given "red")
			try:
				if a+c != 0:
					delta_p_dep_cue = (a/(a+c)) - (b/(b+d))
				else: 
					delta_p_dep_cue = (a/(dummy_zero)) - (b/(b+d))
				stat_dict[x] = delta_p_dep_cue
			except ZeroDivisionError:
				print(a,b,c,d,head_freq, dep_freq)
				sys.exit()
	
		elif stat == "dp_head": # (e.g., getting "red" given "apple")
			try:
				if a+b != 0:
					delta_p_gov_cue = (a/(a+b)) - (c/(c+d))
				else:
					delta_p_gov_cue = (a/(dummy_zero)) - (c/(c+d))
			...
			try:
				logdice_score = 14.0 + math.log2((2.0 * observed)/(head_freq + dep_freq))
				stat_dict[x] = logdice_score
			except ZeroDivisionError:
				print(a,b,c,d,head_freq, dep_freq)
				sys.exit()

		elif stat == "rel_freq": # relative frequency per million words
			rel_freq = (observed/n_bigrams) * 1000000
			stat_dict[x] = rel_freq

		elif stat == "expected": 
			stat_dict[x] = expected

		elif stat == "observed":
			stat_dict[x] = observed
		
		elif stat == "range":
			stat_dict[x] = freq_dict["range"][x]

		elif stat == "fisher_exact":
			_, pvalue = fisher_exact([[a, b],[c, d]])
			stat_dict[x] = pvalue
		
		elif stat == "chi2":
			res = chi2_contingency([[a, b], [c, d]], correction=True)
			stat_dict[x] = res[0]

		elif stat == "phi":
			res = chi2_contingency([[a, b], [c, d]], correction=True)
			stat_dict[x] = res[0]/(a+b+c+d)
		else:
			print("Unknown statistical measure")
			sys.exit()
	return(stat_dict)
